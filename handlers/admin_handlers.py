"""
Admin-only commands (that don't require any other arguments)
"""
...
async def bump(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    """
    target = next_month_str()               # YYYY-MM of next month
    ...
    for baby in bday_babies:
        missing = db.get_missing_wishers(baby["telegram_id"], target)
        for user in missing:
            try:
